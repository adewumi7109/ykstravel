...
{/* bg-primary */}
    <div className='md:flex items-center justify-between bg-[#f7faf8] py-4 md:px-10 px-4'>
      ...
      {open ? <FaXmark/> :  <FaBars/>}
        ...
        <ul className={`md:flex md:items-center text-primary md:pb-0 pb-12 absolute md:static  md:z-auto z-[-1] left-0 w-full  md:w-auto md:pl-0 pl-4 transition-all duration-500 ease-in ${open ? ' top-[4.2rem] opacity-100 bg-[#f7faf8]': 'top-[-490px]'} md:opacity-100 `}>
            {Links.map((link)=>(
